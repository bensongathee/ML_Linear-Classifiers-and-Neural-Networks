...
//			Delta[j] <- g'(in_j) \times (y_j - a_j)
		...
		for(int j = 0; j < outputUnits.length; j++) {
			//in_j
			double in_j = 0.0;
			for(Connection conn : outputUnits[j].incomingConnections) {
				in_j += (conn.weight * conn.src.getOutput());
			}
			//g'(in_j)
			double activationPrime = outputUnits[j].activationPrime(in_j);
			//Delta[j] <- g'(in_j) \times (y_j - a_j)
...
//				Delta[i] <- g'(in_i) * \sum_j w_ij Delta[j]
		for(int l = (this.layers.length-2); l > 0; l--) {  // not sure -> confirm
			...
			for(int i = 0; i < hiddenLayerUnits.length; i++) {
				//in_i
				double in_i = 0.0;
				for(Connection conn : hiddenLayerUnits[i].incomingConnections) {
					in_i += (conn.weight * conn.src.getOutput());
				}
				//g'(in_i)
				...
				double sum = 0.0;
				int n = 0;
				for(Connection conn : hiddenLayerUnits[i].outgoingConnections) {
					sum += (conn.weight * (this.getLayerUnits(l+1)[n++].delta));
				}
				//Delta[i] <- g'(in_i) * \sum_j w_ij Delta[j]
...
//			w_ij <- w_ij + alpha * a_i * delta_j
		for (int l=1; l < this.layers.length; l++) {
			...
			for (int index=0; index < units.length; index++) {
				for (Connection conn : units[index].incomingConnections) {
					conn.weight += (alpha * conn.src.getOutput() * units[index].delta);
				}
			}
		}
					//Backprop
